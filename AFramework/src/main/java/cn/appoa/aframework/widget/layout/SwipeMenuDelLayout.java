...
/**
 * 【Item侧滑删除菜单】 继承自ViewGroup，实现滑动出现删除等选项的效果， 思路：跟随手势将item向左滑动， 在onMeasure时
 * 将第一个Item设为屏幕宽度 【解决屏幕上多个侧滑删除菜单】：内设一个类静态View类型变量
 * ViewCache，存储的是当前正处于右滑状态的CstSwipeMenuItemViewGroup，
 ...
 * <p/>
 * 20160824,fix 【多指一起滑我的情况】：只接第一个客人(使用一个类静态布尔变量) other: 1 菜单处于侧滑时，拦截长按事件 2 解决侧滑时
 * 点击 的冲突 3 通过 isIos 变量控制是否是IOS阻塞式交互，默认是打开的。 4 通过 isSwipeEnable
 * 变量控制是否开启右滑菜单，默认打开。（某些场景，复用item，没有编辑权限的用户不能右滑） 5 2016 09 29 add,，通过开关
 * isLeftSwipe支持左滑右滑 6 2016 10 21 add , 增加viewChache 的
 * get()方法，可以用在：当点击外部空白处时，关闭正在展开的侧滑菜单。 7 2016 10 22 fix , 当父控件宽度不是全屏时的bug。 2016
 * 10 22 add , 仿QQ，侧滑菜单展开时，点击除侧滑菜单之外的区域，关闭侧滑菜单。 8 2016 11 03
 * add,判断手指起始落点，如果距离属于滑动了，就屏蔽一切点击事件。 9 2016 11 04 fix 长按事件和侧滑的冲突。 10 2016 11 09
 * add,适配GridLayoutManager，将以第一个子Item(即ContentItem)的宽度为控件宽度。 11 2016 11 14
 * add,支持padding,且后续计划加入上滑下滑，因此不再支持ContentItem的margin属性。 2016 11 14
 * add,修改回弹的动画，更平滑。 2016 11 14 fix,微小位移的move不回回弹的bug 2016 11 18,fix
 * 当ItemView存在高度可变的情况 Created by zhangxutong . Date: 16/04/24
 */
public class SwipeMenuDelLayout extends ViewGroup {
    ...
    public SwipeMenuDelLayout(Context context) {
        this(context, null);
    }

    public SwipeMenuDelLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SwipeMenuDelLayout(Context context, AttributeSet attrs, int defStyleAttr) {
     ...
     * @param ios
     */
    public SwipeMenuDelLayout setIos(boolean ios) {
        isIos = ios;
        return this;
    }
     ...
     * @return
     */
    public SwipeMenuDelLayout setLeftSwipe(boolean leftSwipe) {
     ...
     * @return
     */
    public static SwipeMenuDelLayout getViewCache() {
        return mViewCache;
    }
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        // 左滑右滑的开关,默认左滑打开菜单
        isLeftSwipe = true;
        TypedArray ta = context.getTheme().obtainStyledAttributes(attrs, R.styleable.SwipeMenuDelLayout, defStyleAttr,
                0);
        int count = ta.getIndexCount();
        for (int i = 0; i < count; i++) {
            int attr = ta.getIndex(i);
            // 如果引用成AndroidLib 资源都不是常量，无法使用switch case
            if (attr == R.styleable.SwipeMenuDelLayout_swipeEnable) {
                isSwipeEnable = ta.getBoolean(attr, true);
            } else if (attr == R.styleable.SwipeMenuDelLayout_ios) {
                isIos = ta.getBoolean(attr, true);
            } else if (attr == R.styleable.SwipeMenuDelLayout_leftSwipe) {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        mHeight = 0;
        int contentWidth = 0;// 2016 11 09
        // add,适配GridLayoutManager，将以第一个子Item(即ContentItem)的宽度为控件宽度
        int childCount = getChildCount();

        // add by 2016 08 11 为了子View的高，可以matchParent(参考的FrameLayout
        // 和LinearLayout的Horizontal)
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (childView.getVisibility() != GONE) {
                ...
                mHeight = Math.max(mHeight, childView
                        .getMeasuredHeight()/*
        ...
        // Log.d(TAG, "onMeasure() called with: " + "mRightMenuWidths = [" +
        // mRightMenuWidths);
        ...
        for (int i = 0; i < count; ++i) {
            ...
            if (child.getVisibility() != GONE) {
                ...
                if (lp.height == LayoutParams.MATCH_PARENT) {
                    ...
                    // text?
                    int oldWidth = lp.width;// measureChildWithMargins
                    // 这个函数会用到宽，所以要保存一下
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        // LogUtils.e(TAG, "onLayout() called with: " + "changed = [" + changed
        // + "], l = [" + l + "], t = [" + t + "], r = [" + r + "], b = [" + b +
        // "]");
        int childCount = getChildCount();
        ...
        // Log.d(TAG, "onLayout() called with: " + "maxScrollGap = [" +
        // maxScrollGap + "], l = [" + l + "], t = [" + t + "], r = [" + r + "],
        // b = [" + b + "]");
    }
        ...
        if (isSwipeEnable) {
            ...
            switch (ev.getAction()) {
                case MotionEvent.ACTION_DOWN:
                    isUserSwiped = false;// 2016 11 03
                    // add,判断手指起始落点，如果距离属于滑动了，就屏蔽一切点击事件。
                    isUnMoved = true;// 2016 10 22 add , 仿QQ，侧滑菜单展开时，点击内容区域，关闭侧滑菜单。
                    iosInterceptFlag = false;// add by 2016 09 11 ，每次DOWN时，默认是不拦截的
                    if (isTouching) {// 如果有别的指头摸过了，那么就return
                        // false。这样后续的move..等事件也不会再来找这个View了。
                        return false;
                    } else {
                        isTouching = true;// 第一个摸的指头，赶紧改变标志，宣誓主权。
                    }
                    mLastP.set(ev.getRawX(), ev.getRawY());
                    mFirstP.set(ev.getRawX(), ev.getRawY());// 2016 11 03
                    // add,判断手指起始落点，如果距离属于滑动了，就屏蔽一切点击事件。
                ...
                case MotionEvent.ACTION_MOVE:
                    ...
                    if (Math.abs(gap) > 10 || Math.abs(getScrollX()) > 10) {// 2016
                        // 09 29
                        // 修改此处，使屏蔽父布局滑动更加灵敏，
                        getParent().requestDisallowInterceptTouchEvent(true);
                    }
                ...
                case MotionEvent.ACTION_CANCEL:
                    ...
                    // LogUtils.i(TAG, "onTouch A ACTION_UP
                    // ACTION_CANCEL:velocityY:" + velocityX);
    ...
    public void smoothExpand() {
		/*
		 * mScroller.startScroll(getScrollX(), 0, mRightMenuWidths -
		 * getScrollX(), 0); invalidate();
		 */
        // 展开就加入ViewCache：
        mViewCache = SwipeMenuDelLayout.this;
    ...
    public void smoothClose() {
        ...
        // LogUtils.d(TAG, "smoothClose() called with:getScrollX() " +
        // getScrollX());
    }
    ...
    // 平滑滚动 弃用 改属性动画实现
	/*
	 * @Override public void computeScroll() { //判断Scroller是否执行完毕： if
	 * (mScroller.computeScrollOffset()) { scrollTo(mScroller.getCurrX(),
	 * mScroller.getCurrY()); //通知View重绘-invalidate()->onDraw()->computeScroll()
	 * invalidate(); } }
	 */
