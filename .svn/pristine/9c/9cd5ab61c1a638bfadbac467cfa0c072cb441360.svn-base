...
/**
 * ===================================== 作 者: 陈嘉桐 445263848@qq.com 版 本：1.0.4
 * 创建日期：2017/4/26 描 述：拍照或录制完成后弹出的确认和返回按钮 =====================================
 */
public class TypeButton extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (button_type == TYPE_CANCEL) {
            ...
            mPaint.setColor(getColor(R.color.type_button_cancel_bg));
            mPaint.setStyle(Paint.Style.FILL);
            canvas.drawCircle(center_X, center_Y, button_radius, mPaint);

            mPaint.setColor(getColor(R.color.type_button_cancel_text));
        ...
        if (button_type == TYPE_CONFIRM) {
            mPaint.setAntiAlias(true);
            mPaint.setColor(getColor(R.color.type_button_confirm_bg));
            mPaint.setStyle(Paint.Style.FILL);
            canvas.drawCircle(center_X, center_Y, button_radius, mPaint);
            mPaint.setAntiAlias(true);
            mPaint.setStyle(Paint.Style.STROKE);
            mPaint.setColor(getColor(R.color.type_button_confirm_text));
            mPaint.setStrokeWidth(strokeWidth);
