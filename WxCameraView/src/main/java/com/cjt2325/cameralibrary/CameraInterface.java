...
/**
 * ===================================== 作 者: 陈嘉桐 版 本：1.0.4 创建日期：2017/4/25 描
 * 述：camera操作单例 =====================================
 */
...
public class CameraInterface implements Camera.PreviewCallback {
        ...
        if (mCamera != null) {
            try {
                ...
                Camera.Size previewSize = CameraParamUtil.getInstance()
                        .getPreviewSize(mParams.getSupportedPreviewSizes(), 1000, screenProp);
                Camera.Size pictureSize = CameraParamUtil.getInstance()
                        .getPictureSize(mParams.getSupportedPictureSizes(), 1200, screenProp);
    ...
    void startRecord(Surface surface, ErrorCallback callback) {
        ...
        videoFirstFrame = Bitmap.createBitmap(videoFirstFrame, 0, 0, videoFirstFrame.getWidth(),
                videoFirstFrame.getHeight(), matrix, true);
        ...
        if (mParams.getSupportedVideoSizes() == null) {
            videoSize = CameraParamUtil.getInstance().getPictureSize(mParams.getSupportedPreviewSizes(), 0, screenProp);
        } else {
            videoSize = CameraParamUtil.getInstance().getPictureSize(mParams.getSupportedVideoSizes(), 0, screenProp);
            videoSize = CameraParamUtil.getInstance().getPictureSize(mParams.getSupportedVideoSizes(), 0, screenProp);
        }
        // Log.i(TAG, "setVideoSize width = " + videoSize.width + "height = " +
        // videoSize.height);
