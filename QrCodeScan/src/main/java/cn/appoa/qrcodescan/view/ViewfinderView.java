...
package cn.appoa.qrcodescan.view;
...
public final class ViewfinderView extends View {
	...
	public void onDraw(Canvas canvas) {
		...
		if (resultBitmap != null) {
			...
			// initDrawableLine(canvas, frame);
			// 画扫描框下面的字
			paint.setColor(Color.WHITE);
			paint.setTextSize(TEXT_SIZE * density);
			paint.setTypeface(Typeface.create("System", Typeface.BOLD));
			String text_bottom = getResources().getString(R.string.scan_text_bottom);
			float textWidthBottom = paint.measureText(text_bottom);
			canvas.drawText(text_bottom, (width - textWidthBottom) / 2,
					(float) (frame.bottom + (float) TEXT_PADDING_TOP * density), paint);
			// 画扫描框上面的字
			// Bitmap bitmap = BitmapFactory.decodeResource(getResources(),
			// R.drawable.icon_qr_code);
			// String text_top =
			// getResources().getString(R.string.scan_text_top);
			// float textWidthTop = paint.measureText(text_top);
			// canvas.drawText(text_top, (width - textWidthTop) / 2,
			// (float) (frame.top - (float) TEXT_PADDING_BOTTOM * density +
			// bitmap.getHeight() / 4), paint);
			// 画扫描框上面的图片
			// canvas.drawBitmap(bitmap, (width - textWidthTop) / 2 -
			// bitmap.getWidth() * 3 / 2,
			// (float) (frame.top - (float) TEXT_PADDING_BOTTOM * density -
	 ...
	 * @param frame
	 */
	private void initDrawableLine(Canvas canvas, Rect frame) {
